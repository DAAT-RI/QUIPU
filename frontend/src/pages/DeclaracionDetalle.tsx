    ...
    <div className="rounded-xl border bg-card p-6 space-y-4">
      ...
      {declaration.categorias && (
        <div className="flex flex-wrap items-center gap-2">
          {declaration.categorias.split(/[;,]/).map((t: string) => t.trim()).filter(Boolean).map((categoria: string) => (
            <span
              key={categoria}
          ...
          <div className="flex-1 min-w-0">
            ...
            {d.categorias && (
              <div className="flex flex-wrap gap-1 mt-1.5">
                {d.categorias.split(/[;,]/).map((t: string) => t.trim()).filter(Boolean).map((categoria: string) => (
                  <span
                    key={categoria}
